...
import { getLessonByModuleKeyAsync } from "./data/moduleLessons.js";
...
function renderLesson(rootNode, lesson, lessonDifficulty, resolvedActivities, total) {
  rootNode.innerHTML = resolvedActivities
    .map((activity, index) => {
...
function initActivities(lesson, resolvedActivities, router) {
...
async function init() {
  const lesson = await getLessonByModuleKeyAsync(moduleKey);

  if (lesson?.title) {
    document.title = `Fluentia Lesson Module - ${lesson.title}`;
  }
  ...
  renderLesson(root, lesson, lessonDifficulty, resolvedActivities, total);
  initActivities(lesson, resolvedActivities, router);
  router.start();
}
