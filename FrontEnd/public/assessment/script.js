...
const matchOptionOrder = {};

async function ensureQuestionsLoaded() {
  if (questionsLoaded) return;

  const sources = [
    window.ASSESSMENT_QUESTIONS_URL,
...
function getMatchOptions(q) {
  if (!matchOptionOrder[q.id]) {
    const options = q.pairs.map((pair) => pair.right);
    for (let i = options.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [options[i], options[j]] = [options[j], options[i]];
    }
    matchOptionOrder[q.id] = options;
  }
  return matchOptionOrder[q.id];
}
...
function renderQuestion() {
  ...
  if (q.type === "reading") {
    ...
    const options = getMatchOptions(q);
    answersHTML = `
          ...
          .map(
            (p) => `
              <div class="match-row">
                ...
                <label class="match-select-wrap">
                  <span class="sr-only">Choose the meaning for ${p.left}</span>
                  <select class="match-select" onchange="selectMatch(${q.id}, '${p.left}', this.value)">
                    <option value="">Select meaning</option>
                    ${options
                      .map(
                        (option) =>
                          `<option value="${option}" ${selected[p.left] === option ? "selected" : ""}>${option}</option>`
                      )
                      .join("")}
                  </select>
                </label>
  ...
  const hasAnswer =
    q.type === "sa"
      ...
      : q.type === "match"
      ? q.pairs.every((pair) => (answers[q.id] || {})[pair.left])
...
function selectMatch(id, left, value) {
  if (!answers[id] || typeof answers[id] !== "object") {
    answers[id] = {};
  }
  answers[id][left] = value;
  renderQuestion();
}
...
function showResults() {
  ...
  questions.forEach((q) => {
    if (q.type === "mc" || q.type === "reading") {
      ...
      const ok = q.pairs.every((pair) => userMatches[pair.left] === pair.right);
      if (ok) score++;
      ...
      feedbacks.push({
        ...
        type: ok ? "correct" : "incorrect",
        note: ok
          ? "All pairs were matched correctly."
          : "One or more matches were incorrect. Review verb-meaning pairings.",
      });
    } else {
      feedbacks.push({
        q: q.text,
  ...
  document.getElementById("results-mount").innerHTML = `
    ...
    <div class="results-actions">
      ...
      <button class="btn-next btn-dashboard" onclick="goToDashboard()">
...
function restartAssessment() {
  ...
  Object.keys(matchOptionOrder).forEach((key) => delete matchOptionOrder[key]);
  current = 0;
